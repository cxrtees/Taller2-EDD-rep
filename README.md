...
* **Árbol Trie de Sufijos (`A_Trie`):** Indexa cada sufijo alfanumérico del nombre de la canción y del artista. Permite realizar búsquedas de subcadenas en tiempo lineal respecto a la longitud del patrón de búsqueda.
* **Árbol AVL (`Arbol_Artistas` y `A_AVL`):** Árboles binarios auto-balanceados por altura. Garantizan operaciones de inserción, eliminación y búsqueda en tiempo logarítmico $O(\log n)$, manteniendo las canciones indexadas alfabéticamente de forma automática.
* **Max-Heap (`A_HeapCanciones` y `A_HeapArtistas`):** Árboles binarios semiordenados representados mediante arreglos dinámicos manuales. Permiten la inserción en tiempo $O(\log n)$ y la extracción del máximo en $O(\log n)$ para la generación de rankings prioritarios.
